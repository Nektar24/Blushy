...
module.exports = {
	name: 'set-picture',
    description: "sets a company's picture",
    permissions : 1,
	execute(message,args) {
        if (args[1] == null || args[1] == ' ' || args[1] == ''){
            message.channel.send(new Discord.MessageEmbed().setDescription('/'+args[0] + ' <company-id> <new-picture-url> '));
            return;
        }
        if (companies[args[1]]){
            companies[args[1]].picture = args[2];
