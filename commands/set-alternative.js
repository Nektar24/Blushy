...
module.exports = {
    ...
    description: "sets a company's alternative growth",
    permissions : 1,
	execute(message,args) {
        if (args[1] == null || args[1] == ' ' || args[1] == ''){
            message.channel.send(new Discord.MessageEmbed().setDescription('/'+args[0] + ' <company-id> <new-alt-growth> '));
            return;
        }
        if (companies[args[1]]){
            companies[args[1]].alternative_growth = parseFloat(args[2]);
